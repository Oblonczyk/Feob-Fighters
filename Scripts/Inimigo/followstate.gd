...
func make_decision() -> void:
	# 🔥 CORREÇÃO: Verifica se o player ainda existe
	if not is_player_valid():
		print("❌ Player não encontrado para tomar decisão")
		return
...
func get_decision_weights() -> Dictionary:
	# 🔥 CORREÇÃO: Verifica se o player existe
	if not is_player_valid():
		return {"approach": 0.0, "circle": 0.0, "retreat": 1.0}
...
func process_physics(delta: float) -> EnemyState:
	# 🔥🔥🔥 CORREÇÃO CRÍTICA: Verifica se o player ainda existe
	if not is_player_valid():
		print("🎯 Player não encontrado, voltando para idle")
...
# 🔥🔥🔥 NOVA FUNÇÃO: Verifica se o player ainda é válido
func is_player_valid() -> bool:
	...
	if not enemy.player.is_inside_tree():
		print("❌ Player is not in scene tree")
		return false
	
	# Verifica se o player está morto (se tiver o método is_alive)
	if enemy.player.has_method("is_alive"):
		if not enemy.player.is_alive:
